...
for i = 1:n_uni_day
    ...
    stream_feature_extraction_cluster(day_of_week(i), 'Water', cur_data, cur_time_vec, win_size, offset, fs, ...
    thre_std, cluster_center, std_dis, S_cov, 'hard');

    % concatenate the features over different days
    output_feature_mat = [output_feature_mat; feature_day_of_week feature_time_of_day feature_mat feature_mat_cluster];
...
output_feature_name = ['DayOfWeek', 'HourOfDay', feature_name, feature_name_cluster];
